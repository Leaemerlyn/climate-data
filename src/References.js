import { Box } from "@mui/material";
import Link from "@mui/material/Link";
import Typography from "@mui/material/Typography";

export function References() {
	return (
		<Box
			maxWidth={1200}
			display="flex"
			alignItems="left"
			flexDirection={"column"}
			gap={4}
			p={6}
			margin={2}
		>
			<Typography variant="h5">References</Typography>
			<div>
				<Typography variant="body1">
					<b>
						Info about effects of natural phenomena on temperature:
					</b>
				</Typography>
				<Link
					href="https://gml.noaa.gov/outreach/info_activities/pdfs/TBI_natural_climate_change.pdf"
					target="_blank"
				>
					https://gml.noaa.gov/outreach/info_activities/pdfs/TBI_natural_climate_change.pdf
				</Link>
			</div>
			<div>
				<Typography variant="body1">
					<b>Explanation of El Nino and La Nina:</b>
				</Typography>
				<Link
					href="https://oceanservice.noaa.gov/facts/ninonina.html"
					target="_blank"
				>
					https://oceanservice.noaa.gov/facts/ninonina.html
				</Link>
			</div>
			<div>
				<Typography variant="body1">
					<b>Temperature drop after Mount Tambora eruption:</b>
				</Typography>
				<Link
					href="https://www.irishtimes.com/opinion/a-volcanic-eruption-with-global-repercussions-an-irishman-s-diary-on-1816-the-year-without-a-summer-1.2760797#:~:text=There%20was%20continuous%20rain%20in,also%20extensive%20suffering%20and%20death"
					target="_blank"
				>
					https://www.irishtimes.com/opinion/a-volcanic-eruption-with-global-repercussions-an-irishman-s-diary-on-1816-the-year-without-a-summer-1.2760797#:~:text=There%20was%20continuous%20rain%20in,also%20extensive%20suffering%20and%20death
				</Link>
				<br />
				<br />
				<Link
					href="https://www.smithsonianmag.com/history/blast-from-the-past-65102374/"
					target="_blank"
				>
					https://www.smithsonianmag.com/history/blast-from-the-past-65102374/
				</Link>
			</div>
			<div>
				<Typography variant="body1">
					<b>Temperature drop after Mount Pinatubo eruption:</b>
				</Typography>
				<Link
					href="https://pubs.usgs.gov/fs/1997/fs113-97/"
					target="_blank"
				>
					https://pubs.usgs.gov/fs/1997/fs113-97/
				</Link>
			</div>
			<div>
				<Typography variant="body1">
					<b>Historical data on El Nino and La Nina episodes:</b>
				</Typography>
				<Link href="https://ggweather.com/enso/oni.htm" target="_blank">
					https://ggweather.com/enso/oni.htm
				</Link>
				<br />
				<br />
				<Link
					href="https://origin.cpc.ncep.noaa.gov/products/analysis_monitoring/ensostuff/ONI_v5.php"
					target="_blank"
				>
					https://origin.cpc.ncep.noaa.gov/products/analysis_monitoring/ensostuff/ONI_v5.php
				</Link>
				<br />
				<br />
				<Link
					href="https://www.cpc.ncep.noaa.gov/products/analysis_monitoring/lanina/enso_evolution-status-fcsts-web.pdf"
					target="_blank"
				>
					https://www.cpc.ncep.noaa.gov/products/analysis_monitoring/lanina/enso_evolution-status-fcsts-web.pdf
				</Link>
			</div>
			<div>
				<Typography variant="body1">
					<b>Current temperature change prediction:</b>
				</Typography>
				<Link
					href="https://news.stanford.edu/2023/01/30/ai-predicts-global-warming-will-exceed-1-5-degrees-2030s/"
					target="_blank"
				>
					https://news.stanford.edu/2023/01/30/ai-predicts-global-warming-will-exceed-1-5-degrees-2030s/
				</Link>
			</div>
			<div>
				<Typography variant="body1">
					<b>Time series info:</b>
				</Typography>
				<Link
					href="https://www.itl.nist.gov/div898/handbook/pmc/section4/pmc4.htm"
					target="_blank"
				>
					https://www.itl.nist.gov/div898/handbook/pmc/section4/pmc4.htm
				</Link>
				<br />
				<br />
				<Link
					href="https://online.stat.psu.edu/stat510/"
					target="_blank"
				>
					https://online.stat.psu.edu/stat510/
				</Link>
				<br />
				<br />
				<Link
					href="https://facebook.github.io/prophet/"
					target="_blank"
				>
					https://facebook.github.io/prophet/
				</Link>
				<br />
				<br />
				<Link
					href="https://facebookresearch.github.io/Kats/"
					target="_blank"
				>
					https://facebookresearch.github.io/Kats/
				</Link>
				<br />
				<br />
				<Link
					href="https://github.com/facebookresearch/Kats?tab=readme-ov-file#forecasting"
					target="_blank"
				>
					https://github.com/facebookresearch/Kats?tab=readme-ov-file#forecasting
				</Link>
				<br />
				<br />
				<Link
					href="https://neptune.ai/blog/arima-vs-prophet-vs-lstm"
					target="_blank"
				>
					https://neptune.ai/blog/arima-vs-prophet-vs-lstm
				</Link>
				<br />
				<br />
				<Link
					href="https://neptune.ai/blog/arima-sarima-real-world-time-series-forecasting-guide"
					target="_blank"
				>
					https://neptune.ai/blog/arima-sarima-real-world-time-series-forecasting-guide
				</Link>
				<br />
				<br />
				<Link
					href="https://otexts.com/fpp2/non-seasonal-arima.html#acf-and-pacf-plots"
					target="_blank"
				>
					https://otexts.com/fpp2/non-seasonal-arima.html#acf-and-pacf-plots
				</Link>
				<br />
				<br />
				<Link
					href="https://www.youtube.com/watch?v=hD13nv8SK6A"
					target="_blank"
				>
					https://www.youtube.com/watch?v=hD13nv8SK6A
				</Link>
				<br />
				<br />
				<Link
					href="https://people.duke.edu/~rnau/411arim.htm"
					target="_blank"
				>
					https://people.duke.edu/~rnau/411arim.htm
				</Link>
				<br />
				<br />
				<Link
					href="https://ilyasbinsalih.medium.com/what-are-acf-and-pacf-plots-in-time-series-analysis-cb586b119c5d"
					target="_blank"
				>
					https://ilyasbinsalih.medium.com/what-are-acf-and-pacf-plots-in-time-series-analysis-cb586b119c5d
				</Link>
			</div>
			<div>
				<Typography variant="body1">
					<b>Blog Post Resources:</b>
				</Typography>
				<Link href="https://pages.github.com/" target="_blank">
					https://pages.github.com/
				</Link>
				<br />
				<br />
				<Link
					href="https://flask.palletsprojects.com/en/3.0.x/installation/#"
					target="_blank"
				>
					https://flask.palletsprojects.com/en/3.0.x/installation/#
				</Link>
				<br />
				<br />
				<Link href="https://mui.com/material-ui/" target="_blank">
					https://mui.com/material-ui/
				</Link>
				<br />
				<br />
				<Link
					href="https://stackoverflow.com/questions/34492197/how-to-render-and-return-plot-to-view-in-flask"
					target="_blank"
				>
					https://stackoverflow.com/questions/34492197/how-to-render-and-return-plot-to-view-in-flask
				</Link>
                <br />
				<br />
				<Link
					href="https://stackoverflow.com/questions/25594893/how-to-enable-cors-in-flask"
					target="_blank"
				>
					https://stackoverflow.com/questions/25594893/how-to-enable-cors-in-flask
				</Link>
			</div>
		</Box>
	);
}
